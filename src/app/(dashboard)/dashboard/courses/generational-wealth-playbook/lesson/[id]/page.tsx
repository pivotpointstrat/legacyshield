  ...
  return (
    <div className="max-w-3xl mx-auto">
      <nav className="flex items-center gap-2 text-sm text-gray-400 mb-8 flex-wrap">
        ...
        <span>›</span>
        <Link href="/dashboard/courses/generational-wealth-playbook" className="hover:text-[#d4a017] transition-colors">Generational Wealth Playbook</Link>
        <span>›</span>
      ...
      <div className="flex items-center justify-between gap-4 pt-6 border-t border-gray-100">
        <div className="flex-1">
          {prevLesson ? (
            <Link href={`/dashboard/courses/generational-wealth-playbook/lesson/${prevLesson.id}`} className="flex items-center gap-3 group">
            ...
            <Link href="/dashboard/courses/generational-wealth-playbook" className="flex items-center gap-3 group">
              <div className="w-9 h-9 rounded-full border-2 border-gray-200 group-hover:border-[#d4a017] flex items-center justify-center transition-colors">
                <span className="text-gray-400 group-hover:text-[#d4a017] transition-colors text-sm">←</span>
              </div>
              <div className="hidden sm:block">
        ...
        <div className="flex gap-1.5">
          {course.lessons.map((l) => (
            <Link
              key={l.id}
              href={`/dashboard/courses/generational-wealth-playbook/lesson/${l.id}`}
        ...
        <div className="flex-1 flex justify-end">
          {nextLesson ? (
            <Link href={`/dashboard/courses/generational-wealth-playbook/lesson/${nextLesson.id}`} className="flex items-center gap-3 group">
