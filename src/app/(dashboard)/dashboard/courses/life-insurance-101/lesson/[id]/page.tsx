  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-400 mb-8 flex-wrap">
        ...
        <span>›</span>
        <Link href="/dashboard/courses/life-insurance-101" className="hover:text-[#d4a017] transition-colors">Life Insurance 101</Link>
        <span>›</span>
        ...
        {lesson.content.map((paragraph, i) => (
          <p
            ...
            dangerouslySetInnerHTML={{
              __html: paragraph
                .replace(/\*\*(.*?)\*\*/g, '<strong class="text-[#0a1628] font-semibold">$1</strong>')
            }}
          />
        ))}
      </div>
      ...
      <div className="flex items-center justify-between gap-4 pt-6 border-t border-gray-100">
        ...
        <div className="flex-1">
          {prevLesson ? (
            <Link
              href={`/dashboard/courses/life-insurance-101/lesson/${prevLesson.id}`}
        ...
        <div className="flex gap-1.5">
          {course.lessons.map((l) => (
            <Link
              key={l.id}
              href={`/dashboard/courses/life-insurance-101/lesson/${l.id}`}
        ...
        <div className="flex-1 flex justify-end">
          {nextLesson ? (
            <Link
              href={`/dashboard/courses/life-insurance-101/lesson/${nextLesson.id}`}
              className="flex items-center gap-3 group"
            >
